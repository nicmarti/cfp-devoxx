...
//                    speakerQandA: String,
  ...
  lazy val speakerQandAAsHtml: String = {
    //val processedMarkdownTest = Processor.process(StringUtils.trimToEmpty(speakerQandA).trim()) // Then do markdown processing
    //processedMarkdownTest
    "<p>" +
      "<br/>" +
...
//    "speakerQandA" -> nonEmptyText(maxLength = 3500),
...
//      speakerQandA,
  ...
  def unapplyProposalForm(p: Proposal): Option[(Option[String], String, String, Option[String], List[String], String,
    String,
    //String,
    String, String, Boolean, String, Option[String], Option[Boolean], Option[String], Option[Seq[Tag]] )] = {
    Option((
      Option(p.id),
      p.lang,
      p.title,
